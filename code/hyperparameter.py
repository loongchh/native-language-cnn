...
def try_model(lr, reg, do, hyper_dir, cuda=0, progbar=True, logger=None):
    args0 = deepcopy(args)
    args0.lr = lr
    ...
    (nlcnn_model, train_loss, train_f1, val_f1) = train(args0, progbar=progbar and (cuda == 0))

    with open(join(hyper_dir, "lr-{:1.1e}_reg-{:1.1e}_do-{:1.2f}_loss-{:.2f}_trainF1-{:.2f}_valF1-{:.2f}.pkl".format( \
        lr, reg, do, train_loss[-1], train_f1[-1], val_f1[-1])), 'wb') as fpkl:
        pickle.dump((train_loss, train_f1, val_f1), fpkl)

    save_path = join(hyper_dir, "lr-{:1.1e}_reg-{:1.1e}_do-{:1.2f}_model-state.pkl".format(lr, reg, do))
...
if __name__ == '__main__':
    ...
    learning_rate = [5e-5, 7e-5, 1e-4, 3e-4, 1e-5, 3e-5, 5e-4, 7e-4]
    regularization = [5e-4, 7e-4, 1e-3, 3e-3, 1e-4, 3e-4, 5e-3, 7e-3]
    ...
    for lr in learning_rate:
        for do in dropout:
            ...
            for (i, reg) in enumerate(regularization):
                p = mp.Process(target=try_model, args=(lr, reg, do, hyper_dir, i))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
